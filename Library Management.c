...
{   headMessage("RETURN BOOK");
    ...
    if((!check_issue1)&&(!check_issue2)&&(!out_issue[i].return_date.dd))
	{
        ...
        if(out_issue[i].return_date.dd>duedate.dd) 
		{
		     printf("\n\n\t\t\t\tPassed the due date!");
		     int day1=duedate.dd;
		     int day2=out_issue[i].return_date.dd;
		     int mon1=duedate.mm;
		     int mon2=out_issue[i].return_date.mm;
		     int year1=duedate.yyyy;
		     int year2=out_issue[i].return_date.yyyy;
		if(day2 < day1)
    {      
	...
	float fine=(day_diff+(mon_diff*30)+(year_diff*365))*0.25;
    printf("\n\n\t\t\t\tDifference: %d years %02d months and %02d days.", year_diff, mon_diff, day_diff);
	printf("\n\n\t\t\t\tTotal Fine to be paid: %0.2f",fine);
		 }
        booksfile = fopen("book.txt", "r+");
        j=0;
